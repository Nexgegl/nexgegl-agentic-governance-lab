...
export default function UseCasesPage() {
  ...
  return (
    <div className="space-y-6">
      <Topbar
        titleAr="سجل حالات الاستخدام"
        titleEn="Use Case Registry"
        subtitleAr="جميع حالات استخدام الذكاء الاصطناعي والوكلاء عبر المؤسسة"
      />
      ...
      <section className="flex flex-wrap items-center gap-3 rounded-xl border border-navy-100 bg-white p-4 shadow-card">
        ...
        <span className="ms-auto text-xs text-navy-400">
          عرض {filtered.length} من {useCases.length} حالة
        </span>
      </section>
      ...
      <section className="overflow-x-auto rounded-xl border border-navy-100 bg-white shadow-card">
        ...
        {filtered.length === 0 ? (
          <div className="px-4 py-10 text-center text-sm text-navy-400">لا توجد حالات استخدام مطابقة للفلاتر المحددة.</div>
        ) : null}
      </section>
    </div>
  );
